...
In this first step, we wrangle the data such that it is usable and provide a brief overview of our dataset and our variables of interest.
...
We first perform chi-square tests and proportion test to get a sense of the data that we are dealing with. Specifically, we ask the following questions: is there a relationship between boroughs and restaurant's inspection grades? Is there a relationship between price and restaurant's inspection grades? More specifically, is price range of the restaurant a factor in 
...
Interpretation: The result of chi-square shows that p-value is less than 0.05, so we reject the null hypothesis at 95% significant level and conclude that the inspection grades of restaurants are significantly different by price scale of restaurants.
...
### Regression Diagnostics and Data Transformation

#### Response Transformation

We first test the normality assumption of the response variable, "rating".
...
Now, we test for the normality assumption of predictors. In particular, we find that the distribution for variable "review_num" is positively skewed. As such, we perform log transformation on this variable.
...
Now, we visualize the effects of categorical variables cuisine type, inspection grade, nad price scales on rating. In particular, we have found that rating has some association with cuisine type, inspection grade and price scale.
...
##### Contiuous Independent Variables
