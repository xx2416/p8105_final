...
In the first step, we wrangle the data and provide a brief overview of our data set and our variables of interest. In particular, we create a numeric variable, percent_critical, that records the percentage of critical flag that a restaurant receives for all violations.
...
df = read_csv("data/inspection_sub_latest_date.csv")
  ...
  group_by(camis) %>%
  mutate(percent_critical = sum(critical_flag == "Critical")/sum(critical_flag != "Critical"))
...
plot(fitted(rating_score), resid(rating_score), col = "dodgerblue",
     pch = 20, cex = 1.5, xlab = "Fitted", ylab = "Residuals")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
plot(rating ~ log(score), data = reg_df, col = "dodgerblue", pch = 20, cex = 1.5)
rating_score = lm(rating ~ log(score), data = reg_df)
abline(rating_score, col = "darkorange", lwd = 2)
plot(fitted(rating_score), resid(rating_score), col = "dodgerblue",
     pch = 20, cex = 1.5, xlab = "Fitted", ylab = "Residuals")
abline(h = 0, lty = 2, col = "darkorange", lwd = 2)
...
=======
         score = log(score),
         rating = rating^2)
>>>>>>> 8e6e31ebce4993561688492d838fc796d31f4da5
```
...
#### Multicollinearity of Dependent Variables
